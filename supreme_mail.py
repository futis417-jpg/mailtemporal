...
CAPACIDADES: INBOUND/OUTBOUND, CUSTOM ALIAS, DOMAIN SELECTOR, AUTO-DESTRUCT.
================================================================================
"""
...
# =================================================================
# [1] SUPREME CONFIGURATION (HARDCODED FOR DEPLOYMENT)
# =================================================================
class SupremeConfig:
    # HARDCODED POR ORDEN DEL DIRECTOR ISHAK
    TOKEN = "8641633728:AAFz8fMKKZivoZ_x_Ad6Zia_IDYrEEqy174"
    ADMIN_ID = 8398522835
    PORT = int(os.getenv("PORT", 8080))
    VERSION = "900.0.1-SUPREME-STABLE"
...
logger.info("🛡️ [DIRECTIVA VEO3] Protocolo Idioma Español ACTIVO. Kernel protegido.")

# =================================================================
# [2] QUANTUM DATABASE V900 (Self-Healing)
# =================================================================
...
class MailEngine:
    ...
    async def create_account(cls, domain, custom_username=None):
        ...
        if await cls.req("POST", "/accounts", json_data={"address": address, "password": password}):
            ...
            if token_data:
                return {"address": address, "password": password, "token": token_data["token"], "domain": domain, "id": token_data["id"]}
        return None
...
async def cron_worker(bot):
    logger.info("🚀 Motor CRON V900 Inyectado. Vigilancia total.")
    while True:
...
class SupremeUI:
    ...
    def create_menu(u):
        ...
        if SupremeConfig.PLANS[u["plan"]]["custom_alias"]:
            ...
            kb.append([InlineKeyboardButton("🔒 Funciones Premium Ocultas", callback_data="app_shop")])
    ...
    def admin_panel():
        ...
        msg = ("👁️ **TERMINAL SUPREMA (DIRECTOR ISHAK)**\n"
               ...
               f"📤 Correos Enviados: `{s['sent_mails']}`\n"
               "──────────────────────")
        kb = [
            [InlineKeyboardButton("👁️ Espionaje", callback_data="adm_users_0"), InlineKeyboardButton("🎫 Acuñar Cupón", callback_data="adm_new_cp")],
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "app_home":
            ...
            if not u["emails"]:
                return await q.edit_message_text("📭 **Matriz vacía.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Retorno", callback_data="app_home")]]))
            ...
            if len(u["emails"]) >= plan_cfg["max_emails"]:
                return await q.answer("Límite estructural alcanzado.", show_alert=True)
            ...
            await q.edit_message_text("🎫 Escribe el código del cupón (ej: TITAN50):")
...
async def text_machine(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "WAIT_COUPON":
        ...
        if not cp or uid in cp["used_by"] or len(cp["used_by"]) >= cp["uses"] or datetime.datetime.now() > parse(cp["expires"]):
            await update.message.reply_text("❌ Cupón inválido o agotado.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Retorno", callback_data="app_home")]]))
            return
        ...
        await update.message.reply_text("🎖️ Plan (PRO, TITAN):")
